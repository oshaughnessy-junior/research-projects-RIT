...
# BARE execute node (NO +SingularityImage).  At job START it detects the real GPU
# compute capability, selects the matching family image, acquires ONLY that image
# (in-place CVMFS/local path -> verbatim; osdf:// -> single stashcp/pelican fetch;
# never the whole family), and execs the real command inside it via nested
# apptainer.  This sidesteps the OSPool failure where a glidein pilot reads an
# expression-valued MY.SingularityImage (ifThenElse(...)) as a LITERAL string and
# cannot build it.
# Env knobs: RIFT_CONTAINER_FORCE_CAP (override detection, testing);
#            RIFT_CONTAINER_APPTAINER_FLAGS (extra `apptainer exec` flags).
set -euo pipefail
LABELS=( @@LABELS@@ )
CAP_MIN=( @@MINS@@ )
CAP_MAX=( @@MAXS@@ )
...
# 1. detect the real GPU compute capability (e.g. 12.0, 6.1) -------------------
...
# 2. pick the matching label (highest cap_min whose [min,max] contains cap) ----
sel=-1
if [ -n "$cap" ]; then
    best_min=-1
    for i in "${!LABELS[@]}"; do
...
# 3. acquire ONLY the matched image -------------------------------------------
...
# 4. exec the real command inside the chosen image ----------------------------
...
def _runtime_image_fields(container):
    """(runtime_path, fetch_url, cap_min, cap_max) for one container.

    runtime_path: where the wrapper looks for / lands the image -- ``./<basename>``
    for a URL (osdf) image, or the verbatim path for an in-place (CVMFS/local)
    image.  fetch_url: the URL to single-fetch at runtime, or ``""`` for in-place
    images (referenced verbatim, never fetched).
    """
...
def build_runtime_selection_wrapper(manifest, inner_command=None):
    """Return the text of an OSG-safe runtime container-selection wrapper script.

    The wrapper is meant to be the Condor ``executable`` (run on the BARE execute
    node, with NO ``+SingularityImage``).  See ``_RUNTIME_WRAPPER_BODY`` for what
    it does at runtime.  Use this when an expression-valued ``MY.SingularityImage``
    cannot be evaluated by the execution site (OSPool glidein pilots read it as a
    literal string and fail).

    ``inner_command`` (optional): a command to run INSIDE the container *before*
    the job's own arguments -- e.g. ``"./ile_pre.sh"`` or an in-container
    executable path.  ``None`` (default) -> the wrapper forwards its arguments
    directly (``apptainer exec <sif> "$@"``).

    The cap->image table is baked from ``manifest`` (the same data the
    ifThenElse/``$$()`` builders use), so this stays a single source of truth.
    """
    ...
    for c in manifest["containers"]:
        ...
        # No min -> open-ended-low catch-all (matches any cap); no max -> open-ended-high.
    ...
    def _arr(values):
        # Bash array literal of double-quoted elements.  Values here are image
        # paths / numbers / labels with no embedded quotes.
