...
parser.add_argument("--pipeline-builder",default=None,choices=["BasicIteration","AlternateIteration","BasicMultiApproxIteration","Hyperpipe"],help="Explicitly select the low-level iteration builder, as a drop-in hot-swap for side-by-side A/B testing. The create_event_parameter_pipeline_* names select those builders directly; Hyperpipe is opt-in and uses the generic .dat pipeline with ILE as its indexed-grid MARG evaluator. Overrides the implicit --use-subdags routing. If unset, the builder is chosen by --use-subdags (Alternate) vs. the default (Basic).")
...
if any(force_grids) and not opts.assume_hyperbolic:
    parser.error("Using --force-scatter-grids, --force-plunge-grids, or "
                 "--force-zoomwhirl-grids requires --assume-hyperbolic!")
...
#    config.read(opts.use_ini)
    ...
    if _use_hpip_pp:
        ...
        _cols = _hpio.build_column_list(
            ...
            use_eob_parameters=opts.use_EOB_parameters,
            use_hyperbolic=opts.assume_hyperbolic)
...
elif opts.use_gwsurrogate and 'NRHybSur' in opts.approx and 'Tidal' not in opts.approx:
        line += " --rom-group {} --rom-param NRHybSur3dq8.h5 --approx {} ".format(sur_location_prefix,opts.approx)
elif opts.use_gwsurrogate and 'NRHybSur' in opts.approx and 'Tidal' in opts.approx:
