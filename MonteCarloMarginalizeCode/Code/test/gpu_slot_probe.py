...
THE DEFECT IT EXISTS FOR.  ``pytest.importorskip('cupy')`` skips on ModuleNotFoundError and
RE-RAISES every other ImportError.  The IGWN CVMFS environment ships the cupy PACKAGE on
every CIT host, so cupy always exists in the ModuleNotFoundError sense; on a host with no
CUDA driver the import gets as far as ``libcuda.so.1: cannot open shared object file``,
which is a plain ImportError.  importorskip therefore turns a GPU-less host into a test
FAILURE.  Measured on ldas-grid 2026-09-18 (CVMFS igwn python 3.11, cupy 12.0.0, pytest
9.1.1): both callers failed there and read as "my branch broke the GPU path".  GitHub's
runners have no cupy installed at all, get the clean ModuleNotFoundError, and skip -- so CI
structurally cannot see this, and it bites exactly on the hosts CLAUDE.md sends numpy-backend
gates to.
...
def probe_verdict():
    ...
    try:
        ...
        line = "the GPU probe could not be run: %s" % _flat(e)
...
def no_gpu(reason):
    ...
    lane is a green report for a lane that never ran.  Read from the ambient environment on
    purpose -- the same rule is applied by .travis/test-integrate.sh over the skip lines, but
    a rule that lives only in the shell does not survive someone running `pytest <file>` by
    hand on the GPU node to reproduce a CI failure.
    """
...
def cupy_or_skip():
    ...
    try:
        ...
        # Reached only if this process disagrees with the child that just succeeded.
