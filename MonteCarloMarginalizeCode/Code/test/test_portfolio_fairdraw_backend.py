...
Background (the bug these tests lock down).  integrate_log deliberately forces the
portfolio onto the HOST -- `self.xpy = numpy; xpy_here = numpy; xpy = numpy` at the top
of the function -- because the portfolio aggregates draws its members produced on their
own backends.  The fair-draw block at the end then did
...
`identity_convert_togpu` is the MODULE-GLOBAL, which is `cupy.asarray` whenever cupy
imports -- independently of self.xpy.  So ln_wt went to the DEVICE, `numpy.exp` dispatched
through cupy's `__array_ufunc__` and handed back a cupy array, and numpy.random.choice was
then asked to read a device array as `p`:
...
extrinsic samples are written at all.  `--sampler-method portfolio` was therefore
unusable on any GPU host.  Reproduced 2026-08-11 on ldas-pcdev13 with the extrinsic-
collapse demo at rho_net 146.8 (run/logs/ralphpf_fixed_2311.log): every replicate of
`--sampler-portfolio AV,GMM` died at mcsamplerPortfolio.py:1883.  Pre-existing -- the
pinned 9887838d tree fails identically.

The identical defect was fixed earlier in mcsamplerAdaptiveVolume's fair-draw block; this
is the port, in two parts:
...
class _XpyDeviceChoice(object):
    ...
    integrate_log overwrites self.xpy with the module-global `numpy`, so the only way to
    reach the draw is to patch that binding.  Everything delegates to real numpy except
    random.choice.  `asnumpy` is present because statutils.init_log takes a device branch
    for any xpy that is not the numpy module itself -- cupy supplies it too.
    """
...
def test_the_fairdraw_block_does_not_reach_for_the_module_global_backend():
    ...
    assert 'identity_convert_togpu' not in block, \
        ('the fair-draw block calls identity_convert_togpu again.  That is the module-global '
         '(cupy.asarray when cupy imports), while integrate_log forces self.xpy to numpy -- '
         'the mismatch aborts every portfolio ILE run on a GPU host.')
