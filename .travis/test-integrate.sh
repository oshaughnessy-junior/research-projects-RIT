...
#
# Five subprocess arms (~70 s total), not a static check: the crash is in a flat driver
# script with no importable export function, and the value written is as much the point as
# the exit code -- exporting exp(lnL) or log(lnL) also exits 0.  Verified to FAIL on
# b281ccff2 without the fix (acgpu_lnL arm only; the other four arms pass there).
